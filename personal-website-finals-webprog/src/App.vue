  ...
  <div id="app" class="grand-line">
    ...
    <transition name="fade">
      <div v-if="currentView === 'menu'" class="menu-screen">
        <div class="row-nav">
          <button class="menu-item-row pirate-font">🏴‍☠️ Captain’s Deck</button>
          <button class="menu-item-row pirate-font">⚓ The Captain’s Tale</button>
          <button class="menu-item-row pirate-font">📜 Grand Line Training</button>
          <button class="menu-item-row pirate-font">🍖 Crew Pastimes</button>
          <button class="menu-item-row pirate-font">🗡️ Battle Abilities</button>
          <button class="menu-item-row pirate-font">🏆 Bounties & Glory</button>
          <button class="menu-item-row pirate-font">🕊️ Message</button>
        </div>
      </div>
    </transition>
...
export default {
  ...
  data() {
    return {
      showPoster: false,
      isLoading: false,
      currentView: 'main' // Timeline: main -> trap -> menu
    }
  },
  methods: {
    triggerTrap() {
...
body { background-color: #000; overflow: hidden; font-family: 'Segoe UI', sans-serif; }
...
.overlay.trap-active { background: rgba(0, 0, 0, 0.7); }
...
.fruit-container:hover .eat-btn { background: #ffcc00; }
...
/* FINAL MENU ROW STYLING */
.menu-screen {
  position: absolute; top: 0; left: 0; width: 100vw; height: 100vh;
  display: flex; align-items: flex-end; padding-bottom: 50px;
}
.row-nav {
  display: flex; flex-direction: row; gap: 15px; padding-left: 5%;
}
.menu-item-row.pirate-font {
  font-family: 'Bangers', cursive; padding: 12px 20px;
  background: rgba(0, 0, 0, 0.85); border: 2px solid #f4d03f;
  color: #f4d03f; font-size: 1.2rem; cursor: pointer; transition: 0.3s;
  white-space: nowrap;
}
.menu-item-row:hover {
  background: #f4d03f; color: black; transform: translateY(-10px);
}

.action-btns { display: flex; gap: 15px; margin-top: 20px; }
.marine-btn { background: #002d5a; color: white; border: 1px solid white; padding: 8px 15px; cursor: pointer; font-weight: bold; }
.marine-btn:hover { background: #ff0000; border-color: #ff0000; }
.hero-title-img { width: 100%; max-width: 500px; animation: titleZoom 5s infinite; }
.pirate-description { color: white; margin: 25px 0; max-width: 550px; }
.pirate-btn { background: #f4d03f; padding: 12px 35px; border: 4px solid #5d4037; font-weight: 900; cursor: pointer; font-family: 'Bangers'; font-size: 1.4rem;}
